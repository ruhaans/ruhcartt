...
type OrderItem = { id:number; product:number; product_name:string; price:number; quantity:number; subtotal:number };
type Order = { id:number; shipping_address:string; total:number; items: OrderItem[]; created_at?: string };
...
export default function Checkout(){
  ...
  const payNow = async () => {
    ...
    try {
      ...
      if (!ok) { setErr("Failed to load Razorpay."); setPlacing(false); return; }

      // 3) Open Razorpay Checkout
      // Use `any` typing here to avoid TS issues if you didn't add the razorpay.d.ts yet.
      const options: any = {
        key: orderInit.key,
    ...
    return (
      <div className="container">
        <div className="card">
          ...
          <p className="text-muted">Order ID: <b>#{order.id}</b></p>
          <div style={{marginTop:12}}>
            {order.items.map(it=>(
              <div key={it.id} style={{display:"grid", gridTemplateColumns:"1fr auto auto", gap:12, padding:"8px 0", borderBottom:"1px solid #1c1f26"}}>
                ...
                <div style={{fontWeight:700}}>₹{Number(it.subtotal).toFixed(2)}
</div>
              </div>
            ))}
            <div style={{display:"flex", justifyContent:"space-between", marginTop:16}}>
              <span>Shipping</span><span className="text-muted">{order.shipping_address || "—"}</span>
            </div>
            ...
            <div style={{display:"flex", justifyContent:"flex-end", gap:12, marginTop:16}}>
              ...
              <Link className="btn btn-primary" to="/products">Done</Link>
            </div>
          </div>
        </div>
      </div>
